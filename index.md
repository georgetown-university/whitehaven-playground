...
documentation:
  ...
  - id: deck
    title: Card Decks
    content: |
      Card Decks group several Cards together, for when a collection of callouts is required. Any of the Card styles  can be used inside the Card Deck, as long as the outer container is a <em>div.card-deck</em> and the inner boxes are <em>div.card</em>. There is no limit to the number of Cards that can be placed inside a Card Deck, but it works best with a minimum of <strong>two</strong>. There will be a maximum of three cards per row.
---